...
for i in range(0, len(P2_data)):
    Zcord.append(float(P2_data[i][-1])) 
    Ycord.append(float(P2_data[i][-2]))
    Xcord.append(float(P2_data[i][-3]))

P2_atoms = list(zip(Xcord,Ycord,Zcord))
...
for i in range (0,len(P1_atoms)):
    for j in range (0,len(P2_atoms)):
        dst = distance.euclidean(P1_atoms[i], P2_atoms[j])
...
atom_index_set = set(atom_index)  

print("Number of comparisons : ",comparisons)

print("\n List of Atom Numbers of protein2 which overlaps with atleast 1 atom in Protein1 : ", atom_index_set)
